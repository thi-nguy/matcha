...
export const ProfileDrawer = () => {
  ...
  const myLikes = [
    { photoSrc: avatar1, profileLink: "/profile" },
    { photoSrc: avatar2, profileLink: "/profile" },
    { photoSrc: avatar3, profileLink: "/profile" },
  ];
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-[auto_10%] place-items-center border-b pb-4">
        {myLikes.map((oneLike, i) => {
          return (
            <a
              href={oneLike.profileLink}
              className="absolute hover:scale-125 transition-all duration-400"
              style={{ left: `${(i + 1) * 20}px`, zIndex: myLikes.length - i }}
            >
              <img
                className="w-8 h-8 object-cover rounded-lg border-2 border-white "
                loading="lazy"
                src={oneLike.photoSrc}
                alt={`avatar_${i}`}
              />
            </a>
          );
